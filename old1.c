...
create_list(char* str, int str_len) {
    ...
    state_t *new_state, *prev_state = NULL;
    node_t *new_node;
    char* transition_str;
    ...
    for(i=0; i<str_len; i++) {
        ...
        *transition_str = str[i];
        ...
        new_state->outputs[0].tail = list->tail;

        

        
    
        // If there is a previous state, set its first output to the following node
        if(prev_state!=NULL) {
            prev_state->outputs[0].head = new_node;
        }
...
free_list(list_t *list) {
	...
	while (curr) {
        printf("Freeing state: %d, reached by char: %c  -  num outputs: %d\n", curr->state->id, *(curr->str), curr->state->num_outs);
...
*/
void
free_automaton(automaton_t *model) {
...
insert_statement(automaton_t *model, char *statement, int statement_len) {
    ...
    if(curr_state->outputs==NULL) {
        printf("First training input! adding immediately.\n");
        new_list = create_list(statement, statement_len);
        // Add created list to automaton's initial state
    ...
    for(int i=0; i<statement_len; i++) {
        ...
        // Check if the next character of the statement matches any outputs
        for(int out_idx=0; out_idx<curr_state->num_outs; out_idx++) {
            printf("HAFYIUHAFIUAH\n");
            output_arc = curr_state->outputs[out_idx].head;
            printf("Current character: '%c' - output character: '%c'\n", statement[i], *(output_arc->str));
        }

        // Traverse to the next state (via an output node/arc) (if possible)
        if(curr_state->num_outs > 0) {
            printf("\nTraversing to next state (from id=%d to id=%d)\n", curr_state->id, curr_state->outputs[0].head->state->id);
            curr_state = curr_state->outputs[0].head->state;
            printf("-------UPDATED--------CURR_ID=%d NUM_OUTS=%d----------\n", curr_state->id, curr_state->num_outs);
        }
    }
}
